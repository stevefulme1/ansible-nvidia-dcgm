...
"""Ansible module for manage dcgm policies."""
...
---
module: dcgm_policy
short_description: Manage DCGM policies
description:
    - Manage DCGM policies using the NVIDIA REST API.
...
- name: Create a policy
  stevefulme1.nvidia_dcgm.dcgm_policy:
    name: "example-name"
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/policies/{resource_id}")
...
def create_resource(module, client):
    ...
    create_data = {k: v for k, v in {
        "name": params.get("name"), "group_id": params.get("group_id")
    }.items() if v is not None}
    resource = client.post("/v1/policies", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/policies/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/policies/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/policies/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/policies/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/policies/{existing['id']}",
...
def main():
    ...
    if existing is None:
        for req in ['name', 'group_id']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a policy.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, policy=to_dict(resource))
        return
