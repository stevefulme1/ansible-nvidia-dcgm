...
"""Ansible module for manage dcgm gpu groups."""
...
---
module: dcgm_group
short_description: Manage DCGM GPU groups
description:
    - Manage DCGM GPU groups using the NVIDIA REST API.
...
EXAMPLES = r"""
- name: Create a group
  stevefulme1.nvidia_dcgm.dcgm_group:
    name: "example-name"
    state: present
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/groups/{resource_id}")
...
def create_resource(module, client):
    ...
    resource = client.post("/v1/groups", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/groups/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/groups/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/groups/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/groups/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/groups/{existing['id']}",
...
def main():
    ...
    if existing is None:
        for req in ['name']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a group.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, group=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, group=to_dict(existing))
