...
"""Ansible module for manage dcgm configurations."""
...
---
module: dcgm_config
short_description: Manage DCGM configurations
description:
    - Manage DCGM configurations using the NVIDIA REST API.
...
options:
    config_id:
        description:
            - The config id for the config.
        type: str
    group_id:
        description:
            - The group id for the config.
        type: str
    state:
        description:
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/configs/{resource_id}")
...
def create_resource(module, client):
    ...
    resource = client.post("/v1/configs", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/configs/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/configs/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/configs/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/configs/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/configs/{existing['id']}",
...
def main():
    ...
    if existing is None:
        for req in ['group_id']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a config.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, config=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, config=to_dict(existing))
