...
"""Ansible module for retrieve dcgm policy details."""
...
DOCUMENTATION = r"""
---
module: dcgm_policy_info
short_description: Retrieve DCGM policy details
description:
    - Retrieve DCGM policy details using the NVIDIA REST API.
...
- name: Get policy details
  stevefulme1.nvidia_dcgm.dcgm_policy_info:
...
def main():
    ...
    if params.get("policy_id"):
        try:
            resource = client.get(f"/v1/policies/{params['policy_id']}")
            module.exit_json(changed=False, policy=to_dict(resource))
        ...
        try:
            ...
            module.exit_json(changed=False, policies=[to_dict(i) for i in items])
        except NvidiaApiError as exc:
            module.fail_json(msg=str(exc))
